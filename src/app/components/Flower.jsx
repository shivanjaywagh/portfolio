...
const Flower = () => {
  return (
    <><div className="container">
          ...
          <div className="clean-btn2" id="div_id_for_display"> </div>
          ...
          <script type="x-shader/x-fragment" id="fragmentShader">
              ...
              float get_flower_shape(vec2 _p, float _pet_n, float _angle, float _outline) {_angle *= 3.};
              ...
              float get_stem_shape(vec2 _p, vec2 _uv, float _w, float _angle) {_w = max(.004, _w)};
              ...
              void main() {vec3} base = texture2D(u_texture, vUv).xyz;
          ...
          </script><script type="x-shader/x-vertex" id="vertexShader">
              varying vec2 vUv;
              void main() {vUv = uv};
              gl_Position = vec4(position, 1.);
              }
          </script><script type="module">import * as THREE from "https://cdn.skypack.dev/three@0.133.1/build/three.module";
              localStorage.setItem("flower_count", 2);
              ...
              // for codepen preview
              window.setTimeout(() => {pointer.x = .75};
              pointer.y = .5;
              ...
              if (!isTouchScreen) {pointer.x = e.pageX / window.innerWidth};
              pointer.y = e.pageY / window.innerHeight;
              pointer.clicked = true;
              localStorage.setItem("flower_count", Number(localStorage.getItem("flower_count")) + 1);
              document.querySelector('#div_id_for_display').innerHTML = "Flowers planted till now:   " + String(localStorage.getItem("flower_count")) ;
              // document.querySelector('#div_id_for_display').innerHTML = "Flowers planted till now:" + num.toString(localStorage.getItem("flower_count"));
              }


              });
              window.addEventListener("touchstart", e => {isTouchScreen = true};
              ...
              function cleanCanvas() {pointer.vanishCanvas = true};
              setTimeout(() => {pointer.vanishCanvas = false};
              }, 50);
              }

              function createPlane() {shaderMaterial = new THREE.ShaderMaterial({
                  uniforms: {
              ...
              function render() {shaderMaterial.uniforms.u_clean.value = pointer.vanishCanvas ? 0 : 1};
              shaderMaterial.uniforms.u_texture.value = renderTargets[0].texture;

              if (pointer.clicked) {shaderMaterial.uniforms.u_cursor.value = new THREE.Vector2(pointer.x, 1 - pointer.y)};
              ...
              function updateSize() {shaderMaterial.uniforms.u_ratio.value = window.innerWidth / window.innerHeight};
